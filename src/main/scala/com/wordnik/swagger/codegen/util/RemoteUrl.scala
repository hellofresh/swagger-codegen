...
trait RemoteUrl {
	def urlToString(url: String, authorization: Option [AuthorizationValue]): String = {
		...
		try{
			val conn: HttpURLConnection = authorization match {
				case Some(auth: ApiKeyValue) => {
					if(auth.passAs == "header") {
						...
						new URL(url + "?%s=%s".format(URLEncoder.encode(auth.keyName, "UTF-8"), URLEncoder.encode(auth.value, "UTF-8"))).openConnection().asInstanceOf[HttpURLConnection]
					}
					else {
