...
class RecipientsApi(object):
    ...
    def postRecipients(self, **kwargs):
        r"""Post Recipients

        Args:
            
            recipients, Object: List of recipients to create (required)
        ...
        allParams = ['recipients', 'country', 'locale', 'skip', 'take', 'sort']

        params = locals()
        for key, val in params['kwargs'].items():
            if key not in allParams:
                raise TypeError("Got an unexpected keyword argument '%s' to method postRecipients" % key)
            params[key] = val
        del params['kwargs']
        ...
        if 'recipients' in params:
            formParams['recipients'] = params['recipients']
    ...
    def putRecipient(self, **kwargs):
        r"""Update Recipient

        Args:
            
            recipient, Object: Recipient payload (required)
        ...
        Returns: ApiModel\Notification\Recipient\Recipient
        """

        allParams = ['recipient', 'country', 'id', 'locale']

        params = locals()
        for key, val in params['kwargs'].items():
            if key not in allParams:
                raise TypeError("Got an unexpected keyword argument '%s' to method putRecipient" % key)
            params[key] = val
        del params['kwargs']

        resourcePath = '/recipients/{id}'
        resourcePath = resourcePath.replace('{format}', 'json')
        method = 'PUT'
        ...
        if 'recipient' in params:
            formParams['recipient'] = params['recipient']
        ...
        responseObject = self.apiClient.deserialize(response, r'ApiModel\Notification\Recipient\Recipient')
        return responseObject
